...
if [ ! -f "/opt/HammerDB-6.0/scripts/$SCRIPT_NAME" ]; then
  echo "Error: Script '/opt/HammerDB-6.0/scripts/$SCRIPT_NAME' not found."
  exit 1
fi

# Run the specified HammerDB script with unbuffered output
exec /opt/HammerDB-6.0/hammerdbcli auto /opt/HammerDB-6.0/scripts/$SCRIPT_NAME 2>&1
